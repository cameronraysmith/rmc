...
\tikzstyle{l1}=[draw, fill=black!15, text width=5em, 
...
\begin{tikzpicture}
    ...
    \path [draw, ->] (m1.-15) -- node [below] {$I_1$} 
        (m2.165) ;
    ...
    \draw [->] (m1.15) -- +(6.44*\edgedist,0) node [right] {$P_1$};
    \draw [->] (m2.15) -- +(3.72*\edgedist,0) node [right] {$P_2$};
   	\draw [->] (m3.15) -- +(\edgedist,0) node [right] {$P_3$};
    \draw [->] (m3.-15) -- +(\edgedist,0) node [right] {$P_4$};
   	    
    \draw [->] (m1.165)+(-\edgedist,0) node[left] {$S_1$} -- (m1.165);
    ...
    \begin{pgfonlayer}{background}
        ...
        \path (m3.south -| m3.east)+(\toplev,-\toplev) node (b) {};
